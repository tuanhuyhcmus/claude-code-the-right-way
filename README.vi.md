...
> **Về token.** Token không phải là ký tự, cũng không hẳn là từ. Nó là đơn vị văn bản mà model dùng để đọc và xử lý ngôn ngữ, và số token phụ thuộc vào tokenizer của từng model. Ví dụ câu *"tôi không muốn đọc bài này nữa"* có thể bị một tokenizer phổ biến tách ra đại khái thành `tôi` | ` không` | ` mu` | `ốn` | ` đ` | `ọc` | ` bài` | ` này` | ` n` | `ữa` — khoảng 10 token cho một câu rất ngắn. Câu tiếng Anh tương đương *"I don't want to read this anymore"* thường chỉ tốn ~8 token. Các tokenizer hiện nay được train chủ yếu trên tiếng Anh, nên text tiếng Việt (nhiều dấu, nhiều ký tự Unicode) thường bị cắt vụn và "đốt" budget nhanh hơn.
...
- **Người dùng Claude Code đã đủ giờ bay để nhận ra ít nhất một trong bốn triệu chứng ở [Section 1](#1-delegate-dont-dictate-âm-thầm-đổ-vỡ-trong-session-dài)** — session anxiety, nhồi nhét, mê tín workflow, nghĩa địa `CLAUDE.md`. Nếu chưa cái nào đánh trúng, bookmark lại và quay lại sau vài tuần dùng thật. Hướng dẫn sẽ không đáp xuống trước khi cơn đau xuất hiện.
...
- **Người sẵn sàng đọc một thư mục `.claude/` như đọc source code.** Đây không phải một framework để cài. Section 6 lập luận rằng thứ thật sự giao được là khả năng mở bất kỳ setup Claude Code nào — của bạn hay của người khác — và thấy cái gì đang thực sự diễn ra bên dưới.
...
Sau ba đoạn, đến lúc chốt lại trước khi đi tiếp. Vì nếu đến đây bạn vẫn chưa thấy *tổ chức lại cách dùng Claude là quan trọng và cần thiết*, thì không nên đi tiếp — hoặc tôi sai (không có chuyện đó), hoặc bạn sai.

Nếu có một câu duy nhất tổng kết ba đoạn đầu: vấn đề không nằm ở chỗ Claude thông minh đến đâu — nó nằm ở *cơ chế LLM chạy tại inference time*. Đoạn 1 cho bạn bốn triệu chứng của việc không biết Claude đang nhớ gì. Đoạn 2 cho bạn hai lệnh để đo chuyện đó. Đoạn 3 cho bạn ba fact nói vì sao bigger model hay bigger window không sửa được gốc. Cả ba gộp lại thay đổi **kỳ vọng** của bạn về việc Claude "nhớ" công việc: nó có một context window mọi thứ bạn load vào đều tranh nhau, nạp lại từ đầu ở mỗi turn, và bị tóm tắt âm thầm khi chạm ngưỡng giới hạn.
...
- **Skill và subagent** đóng gói kiến thức *thủ tục* — *"khi thấy X, làm Y₁ → Y₂ → Y₃"* — và offer cho model kèm metadata (`description`, `when_to_use`, `paths`) về lúc nào nên với tới. Body không vào context cho đến khi được invoke. Subagent còn isolate công việc của mình trong một context window mới, nên cuộc hội thoại chính không bị ô nhiễm.
...
Đặt tên sáu primitive là nửa dễ. Nửa khó là biết *cái nào* khớp với một mẩu kiến thức cụ thể — và nhận ra rằng chọn sai không phải vấn đề phong cách, mà là một khoản thuế định kỳ trên mọi turn tương lai của mọi session tương lai. Phần còn lại của hướng dẫn là mental model cho việc đặt đâu.
...
- **Explicit trigger là cần gạt an toàn, đặc biệt với model yếu hơn.** Auto-trigger giả định model đủ thông minh để match một prompt với description cho đúng. Trên model nhỏ / rẻ / nhanh hơn, auto-trigger misfire nhiều hơn. Ép `/command` bỏ phần đoán.
...
Pattern này có một cạnh sắc: nó phụ thuộc hoàn toàn vào việc body skill biết *nơi để tìm* file context. Nếu convention là implicit — *"đọc plan trong `.planning/`"* trong một số skill, *"load task từ `./.task/`"* trong skill khác — bạn sẽ nhận silent failure: skill invoke, cố đọc một path không tồn tại, và fabricate thay vì halt. Phiên bản hoạt động được của pattern này đặt tên convention cho path context để cả framework tôn trọng, và tốt hơn nữa là nhắc tới nó trong description của skill để user biết phải chuẩn bị gì trước khi invoke.
...
Tất cả những gì trong hướng dẫn này đều đang xây một cơ: khả năng nhìn vào một setup Claude Code — của bạn hay của ai khác — và thấy thực sự cái gì đang diễn ra bên dưới.
