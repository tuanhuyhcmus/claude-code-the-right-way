...
Trước khi nói tới Claude Code cụ thể, có một lớp nền chung cho mọi chatbot và agentic tool dựa trên LLM. Phần này dành cho cả người không dùng Claude — bạn có thề đọc để hiểu cách một chat AI hay Agentic AI hoạt động và Nếu chưa thấm, mọi thứ phía sau liên quan tới cách dùng Claude code sẽ rất khó để hiểu.
...
### Mọi thứ xoay quanh việc enrich context

Tất cả những thứ gọi là *prompt engineering* trước đây khi làm việc với AI chat — và nay với agentic AI đã được tiêu chuẩn hoá thành **memory, skills, rules, agents, hooks**... — mục đích cuối cùng đều cùng một chuyện: cải thiện cái context kia, để mỗi turn gửi lên đầy đủ ngữ cảnh hơn. Với ngữ cảnh tốt hơn đó, AI Model (gpt5, minimax2.7, claude opus 4.6, Qwen 3.5...) reasoning rồi trả về kết quả **CÓ THỂ** sẽ tốt hơn.
...
- **Chỉnh sửa model enterprise** (gpt5 của OpenAI, sonnet/opus của Anthropic) là *không thể* — đó là giải pháp đóng. Bù lại, chúng xịn.
...
Tôi là developer và không hay chấp nhận những thứ tự nhiên chạy được, nên có một fact tôi rất thích POC: khi dùng các ecosystem có sẵn (Claude Code, Antigravity), mọi thứ đã bị che hết để end-user thấy như ma thuật. Cứ thử pair một client với một model **không có khả năng gọi tool** xem — nó phế như chatbox cũ. Còn ngược lại, một model không có khả năng trả về hướng dẫn để client gọi tool cũng phế nốt — y như LLM giai đoạn đầu, chỉ thuần generative text.
...
Đó không phải *best practice*, đó chỉ là kinh nghiệm cá nhân — không có gì là chân lý cả. **Đoạn ở trên vừa rồi mới là chân lý.**
...
3. [**Các primitive**](#3-các-primitive--chỗ-bền-vững-nằm-ngoài-compact) — sáu chỗ bền vững để đặt kiến thức mà `/compact` không viết lại được: CLAUDE.md, rule, skill, hook, memory, subagent.
4. [**Cơ chế placement**](#4-placement--ba-trục-hai-kiểu-trigger-một-decision-tree) — ba trục chi phí, ranh giới tin cậy description-vs-body, trigger explicit-vs-implicit, cách đặt nội dung rule, skill-như-template, và một decision tree cho việc mỗi mẩu kiến thức sống ở đâu.
5. [**Đọc bất kỳ framework nào, tự mình**](#5-đọc-bất-kỳ-framework-nào-tự-mình) — cơ bắp mà hướng dẫn này xây: giờ bạn có thể mở bất kỳ thư mục `.claude/` nào và thấy cơ chế đang làm việc thật, không phải magic.
6. [**Sắp tới**](#6-sắp-tới--và-repo-này-không-phải-gì) — docs sắp ra, ranh giới scope, và một lời kêu gọi cộng tác viên cho dashboard context sống.

> Câu hỏi cơ học *"vì sao 1M chứ không phải 1Tb"* được trả lời ở phụ lục [Ba fact về LLM runtime](./docs/llm-mechanics/three-llm-runtime-facts.vi.md) — bắt buộc cho ai chưa thấm window hữu hạn, gửi lại stateless, và self-attention dilution.
...
Đến đây là lúc chốt lại trước khi đi tiếp. Vì nếu đến đây bạn vẫn chưa thấy *tổ chức lại cách dùng Claude là quan trọng và cần thiết*, thì không nên đi tiếp — hoặc tôi sai (không có chuyện đó), hoặc bạn sai.

Nếu có một câu duy nhất tổng kết: vấn đề không nằm ở chỗ Claude thông minh đến đâu — nó nằm ở *cơ chế LLM chạy tại inference time*. Mục 1 cho bạn bốn triệu chứng của việc không biết Claude đang nhớ gì. Mục 2 cho bạn hai lệnh để đo chuyện đó. Phụ lục ba fact nói vì sao bigger model hay bigger window không sửa được gốc. Gộp lại, chúng thay đổi **kỳ vọng** của bạn về việc Claude "nhớ" công việc: nó có một context window mọi thứ bạn load vào đều tranh nhau, nạp lại từ đầu ở mỗi turn, và bị tóm tắt âm thầm khi chạm ngưỡng giới hạn.
...
## 3. Các primitive — chỗ bền vững nằm ngoài `/compact`
...
## 4. Placement — ba trục, hai kiểu trigger, một decision tree
...
## 5. Đọc bất kỳ framework nào, tự mình
