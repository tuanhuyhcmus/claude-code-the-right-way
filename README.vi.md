...
Để bắt đầu, chúng ta xét 1 ngữ cảnh sử dụng hằng ngày khi làm việc với AI (chatbot hay một agentic tool), ban đầu bạn phải nêu vấn đề đang gặp -> model trả lời, thường phải qua vài turn qua lại thì câu trả lời của model mới thực sự bám đúng yêu cầu — coi đây là một quan sát kinh nghiệm, không phải mốc cố định nằm trong cơ chế. 

Để model "hiểu" được vấn đề qua từng turn thì bên dưới thực sự trông như sau: để turn thứ N vẫn "nhớ" được những gì turn 1→N-1 đã nói, client phải gửi lại *toàn bộ hội thoại turn 1→N-1 lên cho model ở mỗi turn, để nó TỰ SUY NGHĨ LẠI TỪ ĐẦU* — nghe ảo không? Và đây là điểm quan trọng nhất về cách LLM hoạt động: *nó stateless* (nôm na cho người dùng non-tech là bạn hỏi tôi, tôi chỉ trả lời và không nhớ câu trả lời, lần sau hỏi lại nhớ nhắc lại câu hỏi cũ và câu trả lời cũ nếu muốn tôi follow được câu chuyện cũ, ko nhắc cũng ok, nhưng tôi sẽ trả lời không liên quan gì tới thứ đã trao đổi trước đó).

Hồi LLM mới ra, tôi cũng háo hức clone một model về chạy trên máy — turn 1 giới thiệu *"tôi tên là…"*, turn 2 hỏi lại *"mày biết tao là ai không"*, bạn biết câu trả lời rồi đấy. Xong hí hửng đi tìm hiểu thì mới vỡ ra: phải gửi cả history cũ lên thì nó mới biết. Đến đây tôi muốn bạn nhớ rằng mỗi câu hỏi bạn hỏi model đều mang theo 1 context (history + câu hỏi tiếp theo (tất nhiên là mang theo vài thứ nữa, bạn có thể tìm kiếm vì mỗi công cụ sẽ khác nhau 1 chút)) → việc kiểm soát context là 1 điều đáng để lưu tâm. Thêm 1 tin xấu là context này có độ lớn hữu hạn — 1M (4/2026).

Tới đây có thể đặt câu hỏi: *"OK vậy tôi cứ chờ context ngày một to lên thì xong, đúng không?"* Có một sự thật cơ bản mà người dùng phổ thông ít biết: con số **1M** đến từ đâu, vì sao không tăng lên 1T cho xong. Câu trả lời cô đặc nằm ở [Mục 2.7](#27-ba-fact-về-llm-runtime); đào sâu hơn ở [self-attention](./docs/llm-mechanics/self-attention.vi.md) và [vì sao context quan trọng](./docs/llm-mechanics/why-context-matters.vi.md). Tin tạm rằng **1M bị giới hạn bởi cơ chế, không phải cái knob nhà phát hành tự xoay lên**.

> **Về token.** Một token không phải ký tự, cũng không hẳn là từ. Nó là đơn vị văn bản mà tokenizer của model cắt input của bạn ra trước khi model thấy, và mỗi model có cách cắt khác nhau. Lấy câu *"tôi không muốn đọc bài này nữa"* làm ví dụ. Nếu cắt thô theo whitespace, bạn được 7 "từ": `tôi` | `không` | `muốn` | `đọc` | `bài` | `này` | `nữa`. Nhưng tokenizer thật không cắt theo whitespace — chúng dùng subword, và một tokenizer phổ biến có thể cắt câu trên thành đại loại `tôi` | ` không` | ` mu` | `ốn` | ` đ` | `ọc` | ` bài` | ` này` | ` n` | `ữa` — khoảng 10 token cho một câu rất ngắn. Câu tiếng Anh tương đương *"I don't want to read this anymore"* thường ~8 token.
...
Vì sao là *CÓ THỂ*? Nếu bạn đã dùng mấy model opensource nhỏ nhỏ, có thể run trên máy tính cá nhân, bạn sẽ hiểu: dù chuẩn bị context tốt tới đâu, việc nó reasoning rồi trả lời được điều gì đó *liên quan* đã là một phép màu — chứ đừng nói tới thông minh hay hợp lý (Chúng ta sẽ không đi sâu vào lý do vì sao model nhỏ hay opensource thường không tốt bằng enterprise ở bài này, bạn có thể tự tìm câu trả lời). Từ *CÓ THỂ* đó cũng là liều thuốc giảm tức giận khi lâu lâu bạn dùng claude opus hay gpt5.x mà nó ngáo.

Tới đây có thể tóm tắt đơn giản và cô đọng rằng, việc chúng ta tương tác với AI (chat hay agentic) suy cho cùng chỉ là chuyện *client gửi prompt (kèm context) lên, server reasoning rồi trả kết quả về* (chi tiết tôi giải thích ở mục ngay dưới). Tôi nói vậy để tách bạch một chuyện: **enrich context là việc người dùng có thể làm — nhưng nó không phải là chuyện CHẮC CHẮN sẽ khiến chất lượng cuối cùng tốt lên**. Dù vậy, đó lại là điều khả dĩ nhất mà người dùng phổ thông có thể làm. Vì hai lựa chọn còn lại đều hơi căng:
...
- **Tự làm việc với model opensource** (deepseek, Qwen...) là *khó* — đa số chúng ta không phải AI engineer, cũng không có phần cứng đủ tốt để kéo về POC, train, fine-tune. (Khả dĩ nhất là RAG — một cơ chế đơn giản và có điểm tương đồng với enrich context) tuy nhiên bài này sẽ không đi sâu.

Hai loại công việc đó dành cho các kỹ sư chuyên về AI. Chúng ta sẽ đi enrich context.
...
Từ đó, việc chọn model phù hợp cho mỗi task trở thành một knob có chủ ý. Bạn có thể downshift sang model rẻ hơn, yếu hơn mà không lo nó đi chệch harness — vì harness là cấu trúc bạn build, không phải sự thông minh của model.
...
3. [**2.7 Ba fact về LLM runtime**](#27-ba-fact-về-llm-runtime) — ba lý do cơ học tại sao window to hơn và model thông minh hơn không xoá được vấn đề.
4. [**2.8 Các primitive**](#28-các-primitive--chỗ-bền-vững-nằm-ngoài-compact) — sáu chỗ bền vững để đặt kiến thức mà `/compact` không viết lại được: CLAUDE.md, rule, skill, hook, memory, subagent.
5. [**2.10 Cơ chế placement**](#210-placement--ba-trục-hai-kiểu-trigger) — ba trục chi phí, ranh giới tin cậy description-vs-body, trigger explicit-vs-implicit, cách đặt nội dung rule, skill-như-template, artifact như state bền vững.
6. [**2.18 Cây quyết định**](#218-cây-quyết-định--kiến-thức-này-sống-ở-đâu) — kỷ luật tối thiểu cho việc đặt kiến thức: cây top-down từ "behavioral hay factual?" tới đúng primitive.
7. [**Phần 3 — Đọc bất kỳ framework nào, tự mình**](#phần-3--đọc-bất-kỳ-framework-nào-tự-mình) — cơ bắp mà hướng dẫn này xây: giờ bạn có thể mở bất kỳ thư mục `.claude/` nào và thấy cơ chế đang làm việc thật, không phải magic.
8. [**Phần 4 — Sắp tới**](#phần-4--sắp-tới-và-repo-này-không-phải-gì) — docs sắp ra, ranh giới scope, và một lời kêu gọi cộng tác viên cho dashboard context sống.
...
Quay lại với mục đích của repo là trao cho Claude đủ cấu trúc bền vững để phần thông minh, biết khám phá của model có thể đáp xuống một nơi có thể dự đoán được, thay vì hết phiên này đến phiên khác lại phải tự suy luận lại cùng những sự thật cũ về codebase của bạn. Ánh xạ vào đây, nó sẽ là làm cho phần việc *gather context* trở nên **stable, stateful** hơn với các bài toán, vấn đề tương tự trong quá khứ, chứ không phụ thuộc vào cảm xúc của người prompt. Hay nói một cách cụ thể hơn:
...
**Cross-check thứ hai: nó có thực sự nắn output không?** Phần trên trả lời câu hỏi *"số token có thật không?"*. Câu sát hơn là: *"những thứ được load đó có thực sự nắn được cách model trả lời không?"* Vì nếu không có skill/rule của bạn, model vẫn sẽ trả về một câu *"gần đúng"*, mà *"gần đúng"* lại rất khó phân biệt với *"đúng"*. Bạn dễ nghĩ framework của mình work mà thực ra nó không tác động gì cả.
...
Bạn không cần hiểu sâu ML để đặt kiến thức đúng chỗ trong Claude Code. Bạn chỉ cần ba fact, ở dạng vừa đủ. Mọi quyết định placement trong phần còn lại của hướng dẫn đều bắt nguồn từ ba fact này.

**Fact 1 — Context window là hữu hạn, và dùng chung.** Opus 4.7 trần ở 1M token. Nghe khổng lồ; thực ra không, vì budget này **dùng chung**, không phải dành riêng cho câu hỏi của bạn. Mọi thứ bên dưới cạnh tranh trong cùng 1M trên mỗi turn: system prompt của Claude Code; mọi tool schema (built-in, MCP, agent definition); mọi `CLAUDE.md`, rule luôn-load, skill description, và auto-memory file; toàn bộ hội thoại trước, mọi tool result, mọi file Claude đã đọc. Một lần đọc file source 2000 dòng có thể ăn 30–50k token. Mười lần đọc như vậy trong session debug dài và bạn đã đốt 5% window trước khi làm việc thật. *1M token là trần, không phải workspace* — workspace dùng được co lại mỗi phút của session.

**Fact 2 — Mỗi request gửi lại toàn bộ hội thoại.** Một window hữu hạn vẫn quản lý được nếu model giữ state giữa các turn. Nó không. LLM inference là **stateless** — client gửi lại toàn bộ hội thoại trước, mọi `CLAUDE.md` đã load, mọi rule, mọi skill description, mọi tool definition, **mỗi turn**, từ turn 1. Hệ quả thực tế: thêm 500 dòng vào `CLAUDE.md` không phải chi phí một lần. Đó là thuế mỗi turn, nhân với mọi turn của mọi session tương lai. Prompt cache giảm bớt khi prefix khớp, nhưng cache chỉ giúp nếu bạn không invalidate prefix — và edit `CLAUDE.md` giữa session sẽ invalidate mọi thứ bên dưới.

**Fact 3 — Context to hơn không có nghĩa focus tốt hơn.** Trả phí mỗi turn cũng quản lý được nếu mỗi token bạn trả đều làm việc như nhau. Nó không. Self-attention — cơ chế model dùng để quyết token nào quan trọng — gán cho mỗi token một weight, và **những weight đó phải sum lại bằng một tổng cố định**. Mỗi token không liên quan thêm vào sẽ giảm weight cho các token liên quan. Context nhiều hơn làm signal mỏng đi, không sắc hơn. Cộng với hiệu ứng "lost in the middle" đã đo được (info nằm giữa prefix dài và suffix dài bị weight thấp hơn), một prompt dài gấp đôi có thể *kém* hiệu quả hơn prompt ngắn nửa, nếu nội dung thêm là noise.

> **Deep dive:** [`docs/llm-mechanics/self-attention.vi.md`](./docs/llm-mechanics/self-attention.vi.md) *(sắp ra)* mô phỏng vì sao đây là vấn đề cơ học, không phải bug có thể fix bằng window to hơn. [`docs/llm-mechanics/why-context-matters.vi.md`](./docs/llm-mechanics/why-context-matters.vi.md) *(sắp ra)* mở rộng cả ba fact với ví dụ về chi phí thực tế.

**Quỹ đạo có sàn.** Ba fact này không tự fix bằng window to hơn — chúng compound với nó. Model ngày càng thông minh; context window ngày càng lớn — 8k → 200k → 1M → cái gì đó tiếp theo. Engineering wins như prompt caching, path-scoped rule, compaction, và subagent isolation đều đẩy trần lên. Nhưng chúng không đổi *hình dạng* của trần: **vấn đề scale theo tham vọng của bạn, không phải theo model.** Ngày một window 10M token tới, bạn sẽ thấy mình muốn nhét 20M token project context vào đó.

**Reasoning không giống kiến thức bền vững.** Một fact quan sát để chốt. Có lần tôi apply một Terraform config bị lỗi. Không cần hand-hold, Claude suy ra URL có thể sai, đi tìm source của provider, không tìm được local (đặt tên trái convention), web-search match gần nhất, tìm ra GitHub repo, đọc source, và phát hiện có dấu `/` thừa cuối. Đỉnh cao. Và tất nhiên tôi không dám đóng session đó. Đúng dự đoán, N turn công việc không liên quan sau, một vấn đề tương tự xuất hiện và Claude phải re-derive nửa cũ từ đầu. **Một model đủ thông minh để *suy ra* câu trả lời đúng một lần vẫn sẽ re-derive nó, không hoàn hảo, mỗi khi context drift hoặc reset.** Trong session dài, attention rot cuối cùng sẽ chôn câu trả lời. Trong session mới, nó không có ở đó từ đầu.

Kết luận section này dẫn đến: tổ chức cái gì cần nhớ, đặt ở đâu, nạp lại lúc nào không còn là tối ưu chi tiết — nó là yêu cầu cơ bản để session dài không đổ vỡ. Mảnh thiếu là một chỗ để làm cho các kết luận đã derive **bám trụ** — nơi `/compact` không viết lại được và session mới không thể không thấy. Đó là việc các primitive Claude Code làm.
...
Một điều cần thành thật thêm: bài viết này ra đời để *tôi* tự trả lời vấn đề của *tôi* — một developer luôn thấy khó chịu khi dùng một giải pháp mà không hiểu cơ chế nó chạy. Nó không phải cho bạn.
...
Cần mở đầu bằng việc bạn có thể đọc định nghĩa gốc là đủ, không cần đọc mục này. Mục này cung cấp thêm lý luận cá nhân về việc các khái niệm như skill, agent... bổ trợ nhau như thế nào trong việc enrich context và các biến thể trộn lẫn có thể có trong thực tế để bạn dễ hình dung. Nếu bạn đọc chương về giá kỹ thì chương này có thể giúp bạn trong việc cân nhắc đặt body (guide để agent làm step by step) ở rules hay body skills, hay body agent... là hiệu quả hơn về mặt lý thuyết.
...
- **Supply-chain risk.** Một skill share từ repo ngoài có thể cặp description lành ("code formatter") với body độc (exfiltrate secret). Claude tin description để route, rồi execute body trong session của bạn. Đây là lý do nên đọc *body* khi adopt skill từ ngoài, dùng `disable-model-invocation: true` cho thứ có side-effect, và để dành niềm tin cuối cùng cho lớp duyệt từng tool call của Claude Code.
...
Điều này quan trọng nhất khi đánh giá các setup nặng framework. Một framework với hàng chục skill được thiết kế tốt có thể embed phần lớn invariant của nó trực tiếp vào body skill / agent và trả gần như bằng 0 thuế mỗi turn — **miễn là user bám theo harness**. Ngay khi họ free-form vào một chủ đề framework không lường trước, các invariant embedded không load, và session âm thầm fallback về bất cứ `CLAUDE.md` base hay rule centralized nào đang có. Trên harness, setup trông vững như bàn thạch; ngoài harness, user thực chất đang làm việc trong một session Claude Code *không rule nào cả* — và họ sẽ không được cảnh báo khi bước qua ranh giới đó.
...
### 2.18 Cây quyết định — "kiến thức này sống ở đâu?"

Trigger trả lời *khi nào* một thứ fire. Placement trả lời *primitive nào* một mẩu kiến thức thuộc về ngay từ đầu. Cái đó xứng có một cây. Bắt đầu từ trên; match đầu tiên thắng.

```
Cái này là behavioral (Claude nghĩ/nói thế nào) hay factual/procedural (Claude làm gì)?
  ├── Behavioral / triết lý / tone cho cả repo
  │     → CLAUDE.md
  │
  └── Factual / procedural — đi tiếp xuống dưới.

Có phải MUST / MUST NOT luôn đúng, bất kể task?
  ├── Máy kiểm được (grep, lint, exit code)? → rule + hook
  └── Chỉ con người enforce?                  → rule

Có phải workflow "khi user nói X, làm Y₁ → Y₂ → Y₃"?
  └── CÓ → skill

Trả lời nó có cần đọc >2000 dòng source, hay chạy việc song song?
  └── CÓ → subagent (cách ly khỏi main context)

Có phải fact về user hay project thay đổi theo thời gian?
  └── CÓ → memory (để Claude tự viết; đừng viết trước)

Nếu đến đây mà chưa match: nó có lẽ thuộc về một artifact được sinh
  (PLAN.md, CONTEXT.md, DECISIONS.md) — không phải bất kỳ primitive luôn-load nào.
```

Cái cây là kỷ luật tối thiểu — mọi thứ trong repo này là cố làm cho nó thành phản xạ thứ hai. Pin nó ở đâu đó team bạn thấy được khi PR thêm skill hoặc rule mới.

---

## Phần 3 — Đọc bất kỳ framework nào, tự mình
...
## Phần 4 — Sắp tới, và repo này KHÔNG phải gì
